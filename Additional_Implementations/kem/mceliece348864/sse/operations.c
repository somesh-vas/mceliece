...
{	
	...
	output_file1 = fopen("cycles_enc.txt", "a");
	...
	fprintf(output_file1, "Cpu cycles for keygen: %lu\n", (unsigned long)(total_cycles));
	fclose(output_file1);
	return 0;
}
...
{	
	FILE *output_file1;
	uint64_t start_cycles, end_cycles, total_cycles = 0;
	output_file1 = fopen("cycles_dec.txt", "a");
    	if (output_file1 == NULL) {
        	perror("Error opening file");
        return 1;
    }
...
)
{
	FILE *output_file;
	uint64_t start_cycles, end_cycles, total_cycles = 0;
	output_file = fopen("keypair.txt", "a");
    	if (output_file == NULL) {
        	perror("Error opening file");
        return 1;
    }
	...
	end_cycles = rdtsc();
        total_cycles += (end_cycles - start_cycles);
	fprintf(output_file, "Cpu cycles for keygen: %lu\n", (unsigned long)(total_cycles));
	fclose(output_file);
	return 0;
}
